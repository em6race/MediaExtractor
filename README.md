...
4. A standard Windows folder selection window will appear. Choose the folder you want to clean.
5. The script will scan the folder and move all media files to a new folder named `Saved_Media_[YourFolderName]` located next to the original folder.
6. Once the transfer is complete, the script will ask if you want to permanently delete the original folder (which now only contains junk). Type `Y` to delete, or close the window to keep it.
